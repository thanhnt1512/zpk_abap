...
*        WHERE partner_grid = @lt_part_gr_map-partner_grid.
  ...
  LOOP AT gt_data ASSIGNING FIELD-SYMBOL(<fs_data>).
    <fs_data>-amount_tt = <fs_data>-amount_fw + <fs_data>-amount_m1 + <fs_data>-amount_m2 + <fs_data>-amount_m3
          + <fs_data>-amount_m4 + <fs_data>-amount_m5 + <fs_data>-amount_m6 + <fs_data>-amount_m7 + <fs_data>-amount_m8
          + <fs_data>-amount_m9 + <fs_data>-amount_m10 + <fs_data>-amount_m11 + <fs_data>-amount_m12 + <fs_data>-amount_m7_last + <fs_data>-amount_m8_last
          + <fs_data>-amount_m9_last + <fs_data>-amount_m10_last + <fs_data>-amount_m11_last + <fs_data>-amount_m12_last.
    ...
    ls_data-amount_m7_last = ls_data-amount_m7_last + <fs_data>-amount_m7_last.
    ls_data-amount_m8_last = ls_data-amount_m8_last + <fs_data>-amount_m8_last.
    ls_data-amount_m9_last = ls_data-amount_m9_last + <fs_data>-amount_m9_last.
    ls_data-amount_m10_last = ls_data-amount_m10_last + <fs_data>-amount_m10_last.
    ls_data-amount_m11_last = ls_data-amount_m11_last + <fs_data>-amount_m11_last.
    ls_data-amount_m12_last = ls_data-amount_m12_last + <fs_data>-amount_m12_last.
  ...
  ENDIF.
  DELETE gt_data WHERE amount_fw IS INITIAL AND amount_m1 IS INITIAL AND amount_m2 IS INITIAL
                   ...
                   AND amount_m12 IS INITIAL AND amount_tt IS INITIAL AND amount_m7_last IS INITIAL
                   AND amount_m8_last IS INITIAL AND amount_m9_last IS INITIAL
                   AND amount_m10_last IS INITIAL AND amount_m11_last IS INITIAL
                   AND amount_m12_last IS INITIAL.
ENDFORM.
...
FORM f_fieldcat .
  ...
  lm_fieldcat 'AMOUNT_M7_last' 'T07 N-1' 'RHCUR' '' 'X' ''.
  lm_fieldcat 'AMOUNT_M8_last' 'T08 N-1' 'RHCUR' '' 'X' ''.
  lm_fieldcat 'AMOUNT_M9_last' 'T09 N-1' 'RHCUR' '' 'X' ''.
  lm_fieldcat 'AMOUNT_M10_last' 'T10 N-1' 'RHCUR' '' 'X' ''.
  lm_fieldcat 'AMOUNT_M11_last' 'T11 N-1' 'RHCUR' '' 'X' ''.
  lm_fieldcat 'AMOUNT_M12_last' 'T12 N-1' 'RHCUR' '' 'X' ''.
...
*  convert amount
  LOOP AT gt_data INTO DATA(ls_data) WHERE kunnr_name NE 'Tổng cộng'.
    ...
    lw_amount = ls_data-amount_m7_last.
    CALL FUNCTION 'ZFM_CONV_AMOUNT_SAP_TO_DISPLAY'
      EXPORTING
        currency        = ls_data-rhcur
        amount_internal = lw_amount
      IMPORTING
        amount_display  = lw_amount.
    IF sy-subrc IS INITIAL.
      ls_data_excel_dtl-amount_m7_last = lw_amount.
    ENDIF.

    lw_amount = ls_data-amount_m8_last.
    CALL FUNCTION 'ZFM_CONV_AMOUNT_SAP_TO_DISPLAY'
      EXPORTING
        currency        = ls_data-rhcur
        amount_internal = lw_amount
      IMPORTING
        amount_display  = lw_amount.
    IF sy-subrc IS INITIAL.
      ls_data_excel_dtl-amount_m8_last = lw_amount.
    ENDIF.

    lw_amount = ls_data-amount_m9_last.
    CALL FUNCTION 'ZFM_CONV_AMOUNT_SAP_TO_DISPLAY'
      EXPORTING
        currency        = ls_data-rhcur
        amount_internal = lw_amount
      IMPORTING
        amount_display  = lw_amount.
    IF sy-subrc IS INITIAL.
      ls_data_excel_dtl-amount_m9_last = lw_amount.
    ENDIF.

    lw_amount = ls_data-amount_m10_last.
    CALL FUNCTION 'ZFM_CONV_AMOUNT_SAP_TO_DISPLAY'
      EXPORTING
        currency        = ls_data-rhcur
        amount_internal = lw_amount
      IMPORTING
        amount_display  = lw_amount.
    IF sy-subrc IS INITIAL.
      ls_data_excel_dtl-amount_m10_last = lw_amount.
    ENDIF.

    lw_amount = ls_data-amount_m11_last.
    CALL FUNCTION 'ZFM_CONV_AMOUNT_SAP_TO_DISPLAY'
      EXPORTING
        currency        = ls_data-rhcur
        amount_internal = lw_amount
      IMPORTING
        amount_display  = lw_amount.
    IF sy-subrc IS INITIAL.
      ls_data_excel_dtl-amount_m11_last = lw_amount.
    ENDIF.

    lw_amount = ls_data-amount_m12_last.
    CALL FUNCTION 'ZFM_CONV_AMOUNT_SAP_TO_DISPLAY'
      EXPORTING
        currency        = ls_data-rhcur
        amount_internal = lw_amount
      IMPORTING
        amount_display  = lw_amount.
    IF sy-subrc IS INITIAL.
      ls_data_excel_dtl-amount_m12_last = lw_amount.
    ENDIF.
